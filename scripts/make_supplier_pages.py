...
<div class="wrap">
  ...
  <div style="margin-top:18px;border:1px solid #f0d9a8;border-radius:10px;padding:12px 14px;background:#fffdf7">
    <h2 id="shipTitle" style="font-size:15px;margin:0 0 4px;color:#b45309">📦 本周发货情况填报</h2>
    <div style="font-size:12px;color:#8a8f98;margin-bottom:8px">请填写本周（周一~周日）<b>实际发货件数</b>；缺货/延期的在备注里写明原因与预计发货日。填完点「提交」，邮件会自动带好。</div>
    <div id="shipBox" style="overflow-x:auto"></div>
    <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:10px">
      <button onclick="submitShip()" style="background:#e82127;color:#fff;border:none;border-radius:8px;padding:9px 20px;font-size:14px;font-weight:700;cursor:pointer">📤 提交本周发货情况</button>
    ...
    <div style="margin-top:12px;padding-top:10px;border-top:1px dashed #f0d9a8">
      ...
      <div style="font-size:12px;color:#8a8f98;margin-bottom:8px">选好本周签收单（照片可多张 / PDF）→ 手机端点「📤 发送到微信/邮箱」直接发我们；电脑端点「⬇️ 下载」后拖进微信。也可直接发到 <b>xufeng_cao@zhenchang.group</b>（邮件可带附件）。照片会自动压缩，不影响看清。</div>
...
function thisWeekRange() {{
  ...
  const start = new Date(today); start.setDate(today.getDate() + (day === 0 ? -6 : 1 - day));  // 本周一
  const end = new Date(start); end.setDate(start.getDate() + 6);                               // 本周日
  return [start, end];
}}
...
let SHIP_ROWS = [];
function monthPlanOf(r) {{
  const arr = (r.daily && r.daily[curMonth]) || [];
  let s = 0; for (let i = 0; i < arr.length; i++) s += (arr[i] || 0);
  return s;
}}
function renderShip() {{
  const rg = thisWeekRange(), s0 = rg[0], e0 = rg[1];
  const withPlan = DATA.filter(function (r) {{ return planFor(r, s0, e0) > 0; }}).length;
  document.getElementById('shipTitle').textContent = '📦 本周发货情况填报 · ' + dstr(s0) + ' ~ ' + dstr(e0) +
    '（本页 ' + DATA.length + ' 个零件，本周有计划 ' + withPlan + ' 个；无计划的也照列，有发就填）';
  SHIP_ROWS = [];
  DATA.forEach(function (r, i) {{
    SHIP_ROWS.push({{i: i, part: r.part, plan: planFor(r, s0, e0), mplan: monthPlanOf(r)}});
  }});
  let h = '<table><thead><tr><th>零件</th><th>本周计划(件)</th><th>' + MONTH_NAMES[curMonth] + '计划(件)</th><th>实际发货(件)</th><th>发货日期</th><th>备注(缺料/延期原因)</th></tr></thead><tbody>';
  SHIP_ROWS.forEach((x, k) => {{
    const pd = x.plan ? x.plan.toLocaleString() : '<span style="color:#c8cbd0">—</span>';
    const md = x.mplan ? x.mplan.toLocaleString() : '<span style="color:#c8cbd0">—</span>';
    h += '<tr><td class="part">' + x.part + '</td><td>' + pd + '</td><td>' + md + '</td>' +
...
function shipLines() {{
  ...
  SHIP_ROWS.forEach((x, k) => {{
    ...
    if (x.plan > 0) {{ tp += x.plan; if (act === null) unfilled++; }}
    ta += (act || 0);
    if (act === null && x.plan === 0 && !(rv && rv.value)) return;   // 无计划且没填的零件不进邮件
    lines.push('• ' + x.part + '｜本周计划 ' + x.plan + '｜' + MONTH_NAMES[curMonth] + '计划 ' + x.mplan + '｜实际 ' + (act === null ? '未填' : act) +
...
function submitShip() {{
  ...
  const body = SUPPLIER + ' 本周发货情况反馈' + NL + NL +
    '■ 供应商：' + SUPPLIER + NL +
...
function copyShip() {{
  const d = shipLines();
  const txt = '【' + SUPPLIER + '】本周发货情况 ' + dstr(d.rg[0]) + '~' + dstr(d.rg[1]) + NL +
...
function sharePod() {{
  ...
  const txt = '【' + SUPPLIER + '】本周发货签收单 ' + POD.length + ' 个文件';
