...
function renderShip() {{
  ...
  document.getElementById('shipTitle').textContent = '📦 上周发货情况填报 · ' + dstr(s0) + ' ~ ' + dstr(e0) +
    '（本页 ' + DATA.length + ' 个零件，上周有计划 ' + withPlan + ' 个；无计划的也照列，有发就填）';
  SHIP_ROWS = [];
  DATA.forEach(function (r, i) {{
    SHIP_ROWS.push({{i: i, part: r.part, plan: planFor(r, s0, e0), mplan: monthPlanOf(r)}});
  }});
  let h = '<table><thead><tr><th>零件</th><th>上周计划(件)</th><th>' + MONTH_NAMES[curMonth] + '计划(件)</th><th>实际发货(件)</th><th>发货日期</th><th>备注(缺料/延期原因)</th></tr></thead><tbody>';
...
function shipLines() {{
  ...
  SHIP_ROWS.forEach((x, k) => {{
    ...
    lines.push('• ' + x.part + '｜上周计划 ' + x.plan + '｜' + MONTH_NAMES[curMonth] + '计划 ' + x.mplan + '｜实际 ' + (act === null ? '未填' : act) +
